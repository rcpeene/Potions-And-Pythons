...
def chooseObject(name,objects,verb=None):
	...
	for n,obj in enumerate(objects):
		...
		Core.Print(f"{n+1}. {obj.nounPhrase()}{strLabel}",color="k")
	...
	choice = Core.InputLoop(prompt,acceptKey=acceptKey,
	refuseMsg="That is not one of the options.",color="w")
	return choice
...
reqSource=None,silent=False,allowRooms=False):
	...
	if reqSource:
		isMatch = lambda obj: Core.nameMatch(reqSource,obj.parent) or \
			Core.nameMatch(reqSource,obj.anchor())
		matches = {match for match in matches if isMatch(match)}
	if my:
		isMatch = lambda obj: Core.player in obj.ancestors() or obj.determiner == "your"
		matches = {match for match in matches if isMatch(match)}

	# if two items are practically identical, only keep one of them
	uniqueMatches = set()
	for obj in matches:
		if not any(obj.objMatch(m) for m in uniqueMatches):
			uniqueMatches.add(obj)

	if len(uniqueMatches) > 1:
		return chooseObject(term,uniqueMatches,verb)
	elif len(uniqueMatches) == 1:
		return uniqueMatches.pop()

	suffix = ""
	if not silent:
		if reqSource:
			det = "an" if reqSource[0] in Data.vowels else 'a'
			suffix += f" in {det} '{reqSource}'"
...
def enforceVerbScope(verb,obj,permitAnchor=True,permitParent=False,permitSelf=False,
permitDistance=False,permitOutside=False):
	if obj is None:
		return True
	...
	if not permitParent and obj is parent:
		...
		return True
	if not permitOutside and obj is not parent and obj.parent not in (parent,Core.player):
		Core.Print(f"You can't {verb} {-obj}, " \
		f"{obj.pronoun}'s {obj.parent.passprep} {-obj.parent}.")
		return True
...
def isMeaningful(noun):
	...
	noun in Core.game.currentroom.allDirs() or \
...
# e.g. ["attack","green","python"] -> ["attack","green python"]
# this algorithm favors the meaningful terms that contain the most words
def nounify(command):
...
# splits a list based on the present of particular elements as delimiters
def splitList(lst,delims):
	...
	while i < len(lst):
		if lst[i] in delims:
			...
			while i < len(lst) and lst[i] in delims:
				i += 1
			if current:
...
# if the term is a given pronoun, returns the name of the object that
...
def tokenize(command,allowSequence=True):
	...
	# remove articles and determiners (and other superfluous words)
	finalcommand = [word for word in splitcommand if word not in Data.superfluous]
	if not allowSequence:
		return finalcommand
	# allow for multiple sequential commands separated by 'and' or 'then'
	seqcommands = splitList(finalcommand,("and","then"))
	commandQueue.extend(seqcommands[1:])
	return seqcommands[0]


def getNoun(prompt):
	return " ".join(tokenize(read(prompt),allowSequence=False))
...
def parse(cmdInput):
	...
	# some directions map to None, we want to avoid these
	if Data.directions.get(prep,None):
		prep = Data.directions[prep]
...
def interpret():
	...
	if verb.startswith("\\") or verb in cheatcodes:
		commandQueue.clear()
		if verb not in cheatcodes:
			Core.Print(command,color='k')
			return False
...
def Diagnostic(command):
	gc.collect()
	sizes = [sys.getsizeof(obj) for obj in Core.game.itemRegistry.values()]
	...
	objInfo = lambda objects: [(sys.getsizeof(obj),type(obj),repr(obj)[:50]) \
	for obj in objects]

	gameObjects = list(Core.game.itemRegistry.values()) + list(Core.world.values()) + \
	[Core.player,Core.world,Core.game]
	gameObjInfo = objInfo(gameObjects)
	gameObjSizes = [info[0] for info in gameObjInfo]
	print(f"Number of objects in registry: {len(Core.game.itemRegistry)}")
	print(f"Number of rooms in world: {len(Core.world)}")
	print(f"Game object sizes (min, avg, max): {min(gameObjSizes)}b," \
	f" {sum(gameObjSizes)//len(gameObjSizes)}b, {max(gameObjSizes)}b")
	...
	print(f"All python object sizes (min, avg, max): {min(pyObjSizes)}b," \
	f" {sum(pyObjSizes)//len(pyObjSizes)}b, {max(pyObjSizes)}b")
...
def Grow(command):
	...
	if obj is None:
		Core.Print("Error: Object not found",color="k")
		return
	newweight = min(obj.weight*val, obj.parent.Size())
	if newweight > obj.parent.Size():
		Core.Print(f"{+obj} can't get any bigger.",color="k")
		return
	Core.Print(f"Growing {-obj} to weight {newweight}.",color="k")
	obj.weight = Core.min1(newweight)
	Core.player.checkStatus()
...
def Set(command):
	...
	if type(getattr(obj,attrname)) is int and type(value) is not int:
		Core.Print(f"Can't set {attrname} to {value}."
		f" Attribute {attrname} is an integer",color="k")
		return False
...
def Shrink(command):
	...
	if obj.weight <= 1:
		...
		return
	newweight = Core.min1(obj.weight // val)
	Core.Print(f"Shrinking {-obj} to weight {newweight}.",color="k")
...
def Zap(command):
	...
	objname = " ".join(command[1:])
	if objname == "all":
		key = lambda x: not (isinstance(x,(Core.Player,Core.Fixture)) or \
		Core.player in x.ancestors())
...
def Commands(*args):
	...
	Core.columnPrint(shortcommands,10,10,color="k")
	Core.Print("\nVerb Commands (Does not include cheat codes and secret commands)",
	color="k")
	Core.columnPrint(actions.keys(),10,10,color="k")
	Core.Print("\nDuring the game, type 'info' for information on the game and" \
	" how to play.",color="k")
	Core.waitInput()
	Core.clearScreen()
...
def Where(*args):
	allLinks = Core.player.parent.allLinks(d=0)
	...
	Core.Print("You could go...")
	for d in sorted(set(displayDirs)):
		Core.Print(d,color="k")
	if "???" in displayDirs:
...
def Attack(dobj,iobj,prep,target=None,weapon=None,weapon2=None):
	...
	if iobj is None:
		if len(Core.player.weapons()) == 0:
			iobj = "fist"
		elif Core.player.gear["right"] == Core.EmptyGear() and \
		Core.player.gear["left"] == Core.EmptyGear():
	...
	if iobj in ("fist","hand"):
		...
		if weapon is None:
			...
			if len(weapons) == 0: Core.Print(f"There is no '{iobj}' in your Inventory.",
			color="k")
	...
	if not isinstance(weapon,(Items.Hand,Items.Foot,Items.Mouth)):
...
def Bite(dobj,iobj,prep):
	...
	if Core.hasMethod(I,"consume"):
		I.consume(Core.player)
		return True
	else:
...
def Break(dobj,iobj,prep):
	...
	if not Core.hasMethod(I,"breaks"):
...
def Climb(dobj,iobj,prep):
	if prep not in ("down","in","into","inside","on","onto","out","off","under","up",
	"upon",None):
		return promptHelp("Command not understood.")
...
def Crouch(dobj,iobj,prep):
	if prep not in ("at","behind","below","beneath","by","in","inside","into","near",
	"on","onto","under","upon",None):
		return promptHelp("Command not understood.")
	if dobj is None:
		dobj = iobj
	...
	if prep in ("below","beneath","behind","under"):
		...
		obj = findObject(dobj,f"crouch {prep}","room")
		if enforceVerbScope(f"crouch {prep}",obj): return False
...
def Define(dobj,iobj,prep):
	...
	if dobj in Data.glossary:
		Core.Print("\n"+Data.glossary[dobj])
		return True
	elif dobj in Items.factory:
		Core.Print(Items.factory[dobj]().desc)
		return True
	elif dobj in Creatures.factory:
		Core.Print(Creatures.factory[dobj]().desc)
		return True
...
def Drink(dobj,iobj,prep):
	...
	if not Core.hasMethod(I,"imbibe"):
		Core.Print(f"You can't drink {-I}.")
		return False
	I.imbibe(Core.player)
	return True
...
def Eat(dobj,iobj,prep):
	...
	if not Core.hasMethod(I,"consume"):
...
def Fly(dobj,iobj,prep):
	...
	if Core.player.posture() != "floating":
		Core.player.platform = None
...
def parseGo(dobj,iobj,prep):
	...
	# we will mark terms when they are assigned to dir,dest,passage
	termsMatched = {t:False for t in (dobj,iobj,prep)}
	...
	# if a term wasn't assigned above and is assigned by any of the following checks, 
	# it will fail during Go()
	# we try to assign anyway to ensure clear errors are given for ambiguous input

	# assign dir if unassigned
	if dir in Data.directions: dir = Data.directions[dir]
	for term in (dobj,iobj):
		if dir is None and term in Data.directions.values():
			dir = term
			termsMatched[term] = True
	if dir is None: dir = prep

	# assign passage if unassigned
	passages = []
	termMaybePassage = lambda t: not (t is None or matchTerm(t,dir) or matchObj(t,dest))
	for term in (dobj,iobj):
		if passage is None and termMaybePassage(term):
			passages = Core.player.surroundings().nameQuery(term,d=1)
			termsMatched[term] = len(passages) > 0
	# if multiple passages matched, ask which one
	if len(passages) == 1:
	...
	# happens after assigning passage so passage takes precedence when names conflict
	for term in (dobj,iobj):
		if dest is None and Core.nameMatch(term,Core.player.parent):
			dest = Core.player.parent
			termsMatched[term] = True

	# ensure given dobj and iobj were parsed and matched to something
	for term, matched in termsMatched.items():
		if not matched and term not in (prep,None):
			promptHelp(f"There is no '{term}' here.")
			return None,None,None,True
	return dir,dest,passage,cancelledAction


def goPosture():
	# previously we limited movement instead of automatically standing up
	# this caused issues when laying/sitting in a Plash
	if not Core.player.riding and Core.player.posture() in ("laying","sitting"):
		Core.player.changePosture("standing")
		Core.waitInput()
	...
	for cond in Data.immobileStatus:
		...
		if Core.player.riding is not None and Core.player.riding.hasStatus(cond):
			...
			if cond in ("laying","sitting"):
				spurRec = f" Try to spur {Core.player.riding.pronoun}."
...
def Go(dobj,iobj,prep):
	...
	preps = ("above","across","away","away from","by","down","through","to","toward",
	"up","in","inside","into","near","on","onto","out","over",None)
	...
	locRemind = ""
	if Core.player.parent is not Core.game.currentroom:
		locRemind = f" You are in {~Core.player.parent}."
	...
	if passage is None and isinstance(Core.player.parent,Core.Portal):
		passage = Core.player.parent
	# print(dir,dest,passage)
	...
	# print(dir,dest,passage)
	if (dest,passage) == (None,None):
		return promptHelp(f"There is no exit leading '{dir}' here.{locRemind}",tip=tip)
	if passage is None and Core.player.parent is dest:
		Core.Print(f"You are already there!")
		return False
	# if dir is not None and passage is not None and dir not in passage.links:
	# 	return promptHelp(f"{+passage} has no exit leading '{dir}'.",tip=tip)
	...
	return promptHelp(f"There is no exit leading to a '{dobj}' here.{locRemind}",tip=tip)
...
def JumpFrom(dobj,iobj,prep):
	print(dobj,iobj,prep)
	if prep not in ("across","around","behind","below","beneath","by","d","down","from",
	"in","inside","into","near","off","on","onto","over","through","to","toward","u",
	"under","up","upon",None):
		return promptHelp("Command not understood.")
...
def Jump(dobj,iobj,prep):
	if prep not in ("across","around","behind","below","beneath","by","d","down","from",
	"in","inside","into","near","off","on","onto","over","out","through","to","toward",
	"u","under","up","upon",None):
		return promptHelp("Command not understood.")
	for cond in Data.immobileStatus:
		if Core.player.hasStatus(cond) and Core.player.riding is None:
			Core.Print(f"You can't jump, you are {cond}.")
	...
	elif prep == "out" and isinstance(Core.player.parent,Core.Portal):
		jumpTo = Core.player.parent
	elif prep is None and dobj is None:
	...
	if jumpTo is None:
		...
		if jumpTo is None: return False
		if jumpTo is Core.player.anchor() and jumpTo is not Core.player.parent:
	...
	if prep in ("across","d","down","off","over","through"):
		if Core.hasMethod(jumpTo,"traverse"):
			...
			return False
	if prep in ("in","inside","into","on","onto","upon","out"):
	...
	else: Core.player.Dismount(posture="crouch",silent=True)
	...
	if prep in ("in","inside","into","on","onto","to","upon"):
		if jumpTo not in Core.player.parent:
			if not Core.player.changeLocation(jumpTo.parent):
				return False
		return Mount(dobj,iobj,prep,M=jumpTo,posture="crouch")
	return True
...
def Lay(dobj,iobj,prep,M=None):
	...
	if prep in ("below","beneath","behind","under"):
		...
		obj = findObject(dobj,f"lay {prep}","room")
		if enforceVerbScope(f"lay {prep}",obj): return False
		Core.Print("You are there.")
...
def Look(dobj,iobj,prep):
	if prep not in ("around","at","down","in","inside","into","on","out","through","up",
	None):
		return promptHelp("Command not understood.")
	...
	if any(Core.nameMatch(dobj,c) for c in Core.game.celestials) or prep == "up":
		return Core.game.LookUp(dobj)
	elif dobj in ("me","myself",Core.player.name):
		Core.Print(f"You are {Core.player.desc}")
		return True
	elif prep == "down" or dobj in ("ground","floor"):
		L = Core.player.parent.floor
	...
	L.describe()
	if Core.hasMethod(L,"examine"):
		L.examine(Core.player)
	return True
...
def Mount(dobj,iobj,prep,M=None,posture=None):
	...
	if prep in ("in","into","inside") and prep != M.occupyprep:
		if not Core.hasMethod(M,"traverse"):
			return Core.Print(f"{+M} cannot be traversed.")
		return M.traverse(Core.player,verb=posture,dir=prep)
...
def Put(dobj,iobj,prep):
	...
	if prep in ("in","into","inside"):
		if not isinstance(R,Core.Container):
			...
			else: Core.Print(f"You put {idet} {I} in {R.nounPhrase(det=rdet)}.")
			return True
	...
	if R in Core.player:
		...
		Core.Print(f"You can't put {-I} {prep} {-R}.")
		return False
	elif I not in R.parent and not R.parent.canAdd(I):
		Core.Print(f"You can't put {-I} {prep} {-R}. There's not enough room.")
		return False
	...
	# outprep = "on" if isinstance(R,Items.Table) else "in"
	if iobj == "here": Core.Print(f"You put your {I.name} here.")
	else: Core.Print(f"You put {idet} {I} on {R.nounPhrase(det=rdet)}.")

	if I is Core.player.carrying:
		Core.player.removeCarry(silent=True)
	I.parent.remove(I)
	if not R.parent.add(I):
		return False
	R.occupy(I)
	return True
...
def Save(dobj,iobj,prep,quick=False):
	if quick:
...
def Sit(dobj,iobj,prep):
	...
	if prep in ("below","beneath","behind","under"):
		...
		obj = findObject(dobj,f"sit {prep}","room")
		if enforceVerbScope(f"sit {prep}",obj): return False
		Core.Print("You are there.")
...
def Sleep(dobj,iobj,prep):
	if prep not in ("behind","below","beneath","in","inside","into","on","onto","over",
	"under","upon","using","with",None):
...
def Stand(dobj,iobj,prep):
	...
	if prep in ("below","beneath","behind","under"):
		...
		obj = findObject(dobj,f"stand {prep}","room")
		if enforceVerbScope("stand near",obj): return False
		Core.Print("You are there.")
...
def Swim(dobj,iobj,prep):
	goPreps = ("above","behind","below","beneath","by","into","near","out","to",
	"toward","under")
	swimPreps = ("across","d","down","in","inside","on","onto","over",
	"through","u","up","upon")

	if dobj in Data.directions: dobj = Data.directions[dobj]
	...
	prep not in swimPreps):
		...
		if dobj is not None:
			return Go(dobj,iobj,prep)
		if isinstance(Core.player.parent,Core.Room) and \
		Core.player.parent.composition in Data.liquids:
			return Go(dobj,iobj,prep)
		if prep in ("above","out","up") and \
	...
	if prep not in goPreps and prep not in swimPreps and prep is not None:
		return promptHelp("Command not understood.")
...
def TakeAllRecur(objToTake):
	if objToTake is Core.player:
		return False
	takenAny = False
	if hasattr(objToTake,"contents") and not getattr(objToTake,"closed",False):
...
def Take(dobj,iobj,prep):
	if prep in ("behind","below","beneath","under"):
		return Hide(dobj,iobj,prep)
	if prep not in ("from","in","inside","off","out","up",None):
		return promptHelp("Command not understood.")
	...
	if dobj in ("all","each","everything","it all","them"):
	...
	if enforceVerbScope("take",objToTake,permitAnchor=False,permitOutside=True):
		return False
	...
	if any(isinstance(anc,Core.Creature) for anc in objToTake.ancestors()) and \
	objToTake not in Core.player.objTree():
...
def Talk(dobj,iobj,prep):
	...
	if not Core.hasMethod(target,"converse"):
...
def Throw(dobj,iobj,prep,maxspeed=None):
	if prep not in ("at","down","in","into","off","on","onto","out","through","to",
	"toward","up",None):
		return promptHelp("Command not understood.")
...
def Touch(dobj,iobj,prep):
	...
	if I.composition in Data.textures:
		Core.Print(Data.textures[I.composition])
	elif I.composition in Data.liquids:
		Core.Print(f"{+I} feels.")
	return True
...
cheatcodes = {
	...
	"\\zap":Zap,
	"\x02":Honor,  # Ctrl-B ☻
	"\x04":lambda _: None, # Ctrl-D ♦ (reserved for future use)
	"\x05":lambda _: Zap(["","all"]),  # Ctrl-E ♣
	"\x10":lambda _: Learn(["",100]), # Ctrl-P ►
	"\x11":lambda _: Fortify([""]), # Ctrl-Q ◄
	"\x12":lambda _: None,  # Ctrl-R ↕ (reserved for future use)
	"\x13":lambda _: Save(None,None,None,quick=True),  # Ctrl-S ‼
	"\x14": lambda _: None,  # Ctrl-T ¶ (reserved for future use) 
	"\x15":lambda _: Pypot(["",1000]),  # Ctrl-U §
	"\x17":lambda _: Imbue(["","flying"]), # Ctrl-W ↨
	"\x18":Grow, # Ctrl-X ↑
	"\x19":Shrink, # Ctrl-Y ↓
}
...
"ligma":lambda _: Core.Print("Ligma balls!"),
...
"size":lambda _: Core.Print("Size:",Core.player.Size()),
"smile":Smile,
...
"where":Where,
"weight":lambda _: Core.Print("Weight:",Core.player.Weight()),
"wink":Wink,
...
# deactivate/turn [off]
# dress/undress/strip
# hold -> take and equip? or assert in inv already?
# spit
...
# flick/tap
# knock/bang
# knock over/tip over/flip over/overturn, topple, pour?
	# related to shove/push?
# sip -> drink or lick?
# insert (key) -> unlock
# lift -> carry
# mix/stir -> brew?
...
# hump -> fuck
# curse/swear
# pee/poop/piss/shit on (soil your pants if they have them on)
# choke/strangle
# paint/draw
# carve/whittle/hew
# chase -> follow?
